        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    resume = customized_resume_path if 'customized_resume_path' in locals() else "Pending"
                    reposted = False
                    questions_list = None
                    ...
                    # Generate customized resume and cover letter if AI is enabled
                    customized_resume_path = default_resume_path
                    cover_letter_path = None
                    
                    # Extract company information for AI processing
                    company_info = extract_company_info()
                    
                    if use_AI and description != "Unknown":
                        try:
                            print_lg("Generating customized resume and cover letter...")
                            
                            # Generate customized resume
                            customized_resume_path = ai_generate_resume(
                                aiClient, 
                                description, 
                                company_info, 
                            ...
                            if isinstance(customized_resume_path, ValueError):
                                print_lg("Failed to generate customized resume, using default resume")
                                customized_resume_path = default_resume_path
                            else:
                                print_lg(f"Customized resume generated: {customized_resume_path}")
                            ...
                            if isinstance(cover_letter_path, ValueError):
                                print_lg("Failed to generate cover letter")
                                cover_letter_path = None
                            else:
                                print_lg(f"Cover letter generated: {cover_letter_path}")
                                
                        except Exception as e:
                            print_lg(f"Error in AI generation: {e}")
                            customized_resume_path = default_resume_path
                            cover_letter_path = None
                    ...
                    uploaded = False
                    # Case 1: Easy Apply Button - Updated selectors for current LinkedIn interface
                    ...
                    easy_apply_selectors = [
                        ...
                        ".//button[contains(@class,'jobs-apply-button') and contains(@aria-label, 'Apply')]",
                    ...
                    for selector in easy_apply_selectors:
                        if try_xp(driver, selector):
                            ...
                            print_lg(f"Found Easy Apply button with selector: {selector}")
                            break
                    ...
                    if easy_apply_found:
                            ...
                            try:
                                ...
                                if not modal:
                                    ...
                                    modal_selectors = [
                                        ...
                                        "//div[contains(@class,'jobs-apply-modal')]",
                                ...
                                if not modal:
                                    raise Exception("Could not find application modal")
                                ...
                                while next_button:
                                    ...
                                    if useNewResume and not uploaded: uploaded, resume = upload_resume(modal, customized_resume_path)
                        ...
                        print_lg("No Easy Apply button found, trying external application...")
...
def main() -> None:
    try:
        ...
        msg = f"\n{quote}\n\n\nBest regards,\nManideep Reddy Eevuri\nhttps://www.linkedin.com/in/manideep-reddy-eevuri-661659268/\n\n"
...
def extract_company_info() -> str:
    '''
    Function to extract company information for AI processing
    '''
    global about_company_for_ai
    try:
        # Try to find company information in the job details
        company_info_elements = driver.find_elements(By.XPATH, "//div[contains(@class, 'jobs-company__box')]//p")
        if company_info_elements:
            about_company_for_ai = company_info_elements[0].text
        else:
            # Try alternative selectors
            try:
                about_company_for_ai = driver.find_element(By.XPATH, "//div[contains(@class, 'jobs-company__info')]").text
            except:
                about_company_for_ai = "Company information not available"
    except Exception as e:
        print_lg(f"Failed to extract company info: {e}")
        about_company_for_ai = "Company information not available"
    
    return about_company_for_ai
