...
def deepseek_generate_resume(
    ...
    '''
    Function to generate a customized resume using DeepSeek based on job description and user information.
    '''
    ...
    try:
        ...
        response = deepseek_completion(client, messages, stream=stream)
...
def deepseek_generate_coverletter(
    client: OpenAI, 
    job_description: str, about_company: str, required_skills: dict, user_information: str = None,
    stream: bool = stream_output,
    save_path: str = None
    ...
    '''
    Function to generate a cover letter using DeepSeek and save it as a .docx file.
    '''
    from docx import Document
    from datetime import datetime
    import os
    ...
    try:
        ...
        messages = [{"role": "user", "content": prompt}]
        cover_letter = deepseek_completion(client, messages, stream=stream)
        ...
        critical_error_log("Error occurred while generating cover letter with DeepSeek!", e)
